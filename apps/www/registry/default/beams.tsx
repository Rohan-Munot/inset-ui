...
const createStackedPlanesBufferGeometry = (
  ...
  for (let i = 0; i < n; i++) {
    ...
    for (let j = 0; j <= heightSegments; j++) {
      ...
      uvs.set([uvXStart, uvY, uvXEnd, uvY], uvOffset)
...
interface SceneContentProps extends ComponentProps {
...
const SceneContent = ({
  ...
  const dynamicBeamHeight = viewport.height

  const validBeamNumber =
    typeof beamNumber === 'number' && beamNumber > 0 ? beamNumber : 12

  const calculatedBeamWidth = viewport.width / validBeamNumber
      ...
      extendMaterial(THREE.MeshStandardMaterial, {
        ...
        vertexHeader: `
          float getPos(vec3 pos) {
            vec3 noisePos = vec3(
              pos.x * 0.5,
              pos.y * 0.5 + time * uSpeed,
              pos.z
            ) * uScale;
 
            float n = cnoise(noisePos) * 1.5;
            return n;
          }
        ...
        material: { fog: true, side: THREE.DoubleSide },
        uniforms: {
          ...
          uScale: scale,
          uBeamHeight: { value: dynamicBeamHeight },
        },
      }),
    [speed, noiseIntensity, scale, dynamicBeamHeight]
  )

  const lightPosition: [number, number, number] = mousePos
  ...
  return (
    <>
      <NoisePlanes
        ...
        width={calculatedBeamWidth}
        height={dynamicBeamHeight}
      />
      <DirLight color={lightColor} position={lightPosition} />
