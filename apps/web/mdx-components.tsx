...
export function getMDXComponents(overrides: MDXComponents = {}): MDXComponents {
  return {
    ...
    LinkedCard: ({ className, ...props }: React.ComponentProps<typeof Link>) => (
      <Link
        className={cn(
          'bg-card text-card-foreground hover:bg-card/80 flex w-full flex-col items-center rounded-xl border p-6 transition-colors sm:p-10',
          className
        )}
        {...props}
      />
    ),
    // Spread overrides last so they can override defaults
    ...overrides,
  };
}

// Export a pre-built object for simple usage: <MDX components={mdxComponents} />
